...
describe("Testing App Component's State />", () => {
  ...
  it(`verify that markNotificationAsRead works as intended,
	verify that the state of the container has been updated correctly`, () => {
    const context = {
      user: {
        ...user,
      },
...
describe("Test mapStateToProps", () => {
  it("should return the right object when passing the state", () => {
    // Define a sample state using fromJS (Immutable Map)
    const state = fromJS({
      uiReducer: {
        isLoggedIn: true,
      },
    ...
    // Assert that the returned object matches the expected structure
